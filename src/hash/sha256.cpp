...
std::string toHex(std::span<const std::uint8_t> bytes) {
  ...
  for (auto b : bytes) os << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(b);
  return os.str();
}
